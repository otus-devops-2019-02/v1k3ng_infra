...
При добавлении ssh-ключей несколькиз пользователей при использовании такой конструкции:
```
  metadata {
...
Если необходимо добавить несколько ключей, можно использовать такую конструкцию:
```
  metadata {
...
Создан второй инстанс для балансировщика нагрузки (reddit-app2). IP-адрес обоих инстансов добавлен в output-переменные. В данный момент закомменчен. Т.е. выводится только IP-адрес балансировщика. Проблемы при создании инстансов методом копипасты будут при больших масштабированиях. 2-3-5 инстансов еще терпимо, но когда счет пойдет на десятки - код станет плохо читабелен.
